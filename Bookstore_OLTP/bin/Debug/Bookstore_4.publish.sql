...
BEGIN
    ;WITH dedup AS (
        SELECT
            ...
            c.[first_name],
            c.[last_name],
            CONCAT(c.[first_name], N' ', c.[last_name]) AS [full_name],
            c.[email],
            a.[street_name],
            a.[street_number],
            a.[city],
            ...
            ROW_NUMBER() OVER (
                ...
                ORDER BY CASE WHEN adds.[address_status] = 'Active' THEN 0 ELSE 1 END
            ) AS rn
        FROM [dbo].[customer] c
    ...
    SELECT [customer_id], [first_name], [last_name], [full_name], [email],
           [street_name], [street_number], [city], [address_status], [country_name]
    FROM dedup
    WHERE rn = 1;
END
GO
